...
%%%%% and plotting for lab6 experiment 1b
%%%%% Here, we will implement the same thing as 1a but add a feature to
%%%%% estimate a mean and scaling of our data streams and re-normalize. 

%the signal scaling will be of the form: ch_scale( A - ch_offset) where A
%is the original EMG signal. 
%we will set ch_offset so that the minimum of our signal is ~0
%we will set ch_scale so that the EMG amplitude is ~EMG_SCALE (a constant
%we define)
...
EMG_SCALE  = 10; %scale range of EMG (once normalized) 
...
for i=1:RUN_LENGTH%FILLIN
    ...
    %we will visualize the mean(|EMG|) for each channel, as before
    %But now we will scale the input: ch_scale*( X - ch_offset)
    emg_inputs(:,i) = (mean(abs(data_chunk), 2) - ch_offsets).*ch_scale; %FILLIN
